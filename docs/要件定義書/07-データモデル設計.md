...
### 7.4 運営管理テーブル（Operator Management）

> **新規追加（2025-12-09）**
>
> SalonTalk AI運営者向けの管理機能用テーブル。一般ユーザー（サロン管理者）とは
> 別の認証・権限体系を持つ。

#### 7.4.1 operator_admins（運営ユーザー）
...
| name | text | NO | - | 表示名 |
| role | text | NO | 'operator_support' | ロール（operator_admin/operator_support） |
| mfa_secret | text | YES | - | MFA用シークレット（暗号化） |
| mfa_enabled | boolean | NO | false | MFA有効化フラグ |
...
#### 7.4.2 audit_logs（監査ログ）

| カラム名 | 型 | NULL | デフォルト | 説明 |
|---------|-----|------|-----------|------|
| id | uuid | NO | gen_random_uuid() | 主キー |
| operator_id | uuid | NO | - | 操作者（FK） |
| action | text | NO | - | 操作種別 |
| target_type | text | NO | - | 対象種別（salon/operator/system） |
| target_id | uuid | YES | - | 対象ID |
| target_name | text | YES | - | 対象名（サロン名など、検索用） |
| details | jsonb | NO | '{}' | 変更詳細（before/after/reason等） |
| ip_address | inet | YES | - | 操作元IPアドレス |
...
-- 'unsuspend'       - サロン再開
-- 'salon_create'    - サロン作成
-- 'operator_create' - 運営ユーザー作成
-- 'operator_update' - 運営ユーザー更新
-- 'operator_delete' - 運営ユーザー削除
-- 'login'           - ログイン
-- 'logout'          - ログアウト
...
CREATE INDEX idx_audit_logs_target_name ON audit_logs(target_name);

-- 複合インデックス（検索最適化）
CREATE INDEX idx_audit_logs_search ON audit_logs(created_at DESC, action, target_type);

-- RLS（運営者のみアクセス可能）
ALTER TABLE audit_logs ENABLE ROW LEVEL SECURITY;

-- operator_adminは全件閲覧可能
CREATE POLICY "audit_logs_admin_select" ON audit_logs
  FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM operator_admins
      WHERE id = auth.uid() AND role = 'operator_admin' AND is_active = TRUE
    )
  );
...
CREATE POLICY "audit_logs_support_select" ON audit_logs
  FOR SELECT USING (
    ...
    AND EXISTS (
      SELECT 1 FROM operator_admins
      WHERE id = auth.uid() AND role = 'operator_support' AND is_active = TRUE
    )
  );
...
> **変更（2025-12-09）**: 運営管理機能に必要なカラムを追加

```sql
-- salonsテーブルに停止関連カラムを追加
