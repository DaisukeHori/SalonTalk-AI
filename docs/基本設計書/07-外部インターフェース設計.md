...
#### 7.2.4 🆕 内部API: デバイス管理（2025-12-09追加）

**エンドポイント一覧**:

| メソッド | パス | 説明 | 認証 |
|---------|------|------|------|
| GET | /api/devices | デバイス一覧取得 | Bearer |
| POST | /api/devices | デバイス登録 | Bearer (owner/manager) |
| GET | /api/devices/:id | デバイス詳細取得 | Bearer |
| PATCH | /api/devices/:id | デバイス更新 | Bearer (owner/manager) |
| DELETE | /api/devices/:id | デバイス削除 | Bearer (owner) |
| POST | /api/devices/:id/activation-code | アクティベーションコード発行 | Bearer (owner/manager) |
| POST | /api/devices/activate | デバイスアクティベート | なし（コードで認証） |
| POST | /api/devices/authenticate | デバイス認証（起動時） | なし（デバイスIDで認証） |
...
interface CreateDeviceResponse {
  ...
  device_name: string;
  status: 'pending';
  activation_code: string;  // 6桁コード
  expires_at: string;       // ISO8601
}
```

**POST /api/devices/activate**（アクティベート）:

```typescript
// リクエスト
interface ActivateDeviceRequest {
  activation_code: string;    // 6桁コード
  device_identifier: string;  // デバイス固有ID
  metadata?: {
    os_version?: string;
    app_version?: string;
    model?: string;
...
**POST /api/devices/authenticate**（デバイス認証）:
...
#### 7.2.5 🆕 内部API: スタッフ声紋（2025-12-09追加）

**エンドポイント一覧**:

| メソッド | パス | 説明 | 認証 |
|---------|------|------|------|
| POST | /api/staffs/voice-register | 声紋登録・更新 | Bearer |
| GET | /api/staffs/:id/voice-status | 声紋登録状態取得 | Bearer |
| DELETE | /api/staffs/:id/voice | 声紋削除 | Bearer |
| POST | /api/sessions/:id/identify-staff | セッションのスタッフ識別 | Bearer (device) |
| POST | /api/sessions/:id/confirm-staff | スタッフ手動確認 | Bearer (device) |

**POST /api/staffs/voice-register**（声紋登録）:

```typescript
// リクエスト（multipart/form-data）
// - audio: WAVファイル（30秒以上）
// - is_additional: boolean（追加サンプルかどうか）

// レスポンス
interface VoiceRegisterResponse {
  success: boolean;
  sample_count: number;     // 累計サンプル数
  quality_score: number;    // 品質スコア (0-100)
  message: string;
}
```

**POST /api/sessions/:id/identify-staff**（スタッフ自動識別）:
...
interface IdentifyStaffResponse {
  ...
  confidence: number;           // 類似度 (0-1)
  confidence_level: 'high' | 'medium' | 'low' | 'none';
  requires_confirmation: boolean; // 低信頼度時true
}
```

**POST /api/sessions/:id/confirm-staff**（スタッフ手動確認）:

```typescript
// リクエスト
interface ConfirmStaffRequest {
  stylist_id: string;           // 確認/修正されたスタッフID
  learn_from_audio?: boolean;   // 声紋学習に使用するか
}
...
#### 7.2.6 🆕 pyannote Server: 埋め込み抽出API（2025-12-09追加）

**POST /extract-embedding**（声紋埋め込み抽出）:
...
// - extract_embeddings: boolean（🆕 話者ごとの埋め込み抽出）
...
interface DiarizationCallbackExtended {
  ...
  speaker_embeddings?: {      // 🆕 extract_embeddings=true時
    label: string;
    embedding: number[];
    duration_ms: number;
  }[];
