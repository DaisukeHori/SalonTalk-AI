...
-- 🆕 更新（2025-12-09）: voice_embedding追加（スタッフ声紋識別用）
-- ------------------------------------------------------------
CREATE TABLE staffs (
  ...
  voice_registered_at TIMESTAMPTZ,       -- 🆕 声紋登録日時
  voice_sample_count INTEGER DEFAULT 0,  -- 🆕 声紋サンプル数（品質向上用）
  is_active BOOLEAN NOT NULL DEFAULT true,
  setup_completed BOOLEAN NOT NULL DEFAULT false, -- 🆕 初回セットアップ完了フラグ
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
...
CREATE TABLE devices (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  salon_id UUID NOT NULL REFERENCES salons(id) ON DELETE CASCADE,
  device_name VARCHAR(100) NOT NULL,              -- 表示名（例: "セット面1 iPad"）
  device_identifier VARCHAR(255),                 -- デバイス固有ID（UDID等）
  status device_status NOT NULL DEFAULT 'pending',
  seat_number INTEGER,                            -- セット面番号（任意）
  last_active_at TIMESTAMPTZ,                     -- 最終アクティブ日時
  activated_at TIMESTAMPTZ,                       -- アクティベーション日時
  activated_by UUID REFERENCES staffs(id),        -- アクティベーション実行者
  metadata JSONB NOT NULL DEFAULT '{}'::jsonb,    -- デバイス情報（OS version等）
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
...
-- ------------------------------------------------------------
-- device_seat_history（デバイス座席変更履歴）
-- 🆕 新規追加（2025-12-09）: 座席番号変更の監査ログ
-- ------------------------------------------------------------
CREATE TABLE device_seat_history (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  device_id UUID NOT NULL REFERENCES devices(id) ON DELETE CASCADE,
  previous_seat_number INTEGER,                    -- 変更前の座席番号（NULL=未設定から変更）
  new_seat_number INTEGER,                         -- 変更後の座席番号（NULL=座席番号を削除）
  changed_by UUID REFERENCES staffs(id) ON DELETE SET NULL, -- 変更実行者
...
-- ------------------------------------------------------------
-- device_activations（デバイスアクティベーションコード）
-- 🆕 新規追加（2025-12-09）: 6桁コードによるデバイス認証
-- ------------------------------------------------------------
CREATE TABLE device_activations (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  device_id UUID NOT NULL REFERENCES devices(id) ON DELETE CASCADE,
  activation_code CHAR(6) NOT NULL,               -- 6桁数字のみのアクティベーションコード
  expires_at TIMESTAMPTZ NOT NULL,                -- 有効期限（発行から24時間）
  used_at TIMESTAMPTZ,                            -- 使用日時
  created_by UUID NOT NULL REFERENCES staffs(id), -- コード発行者
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),

  -- 🆕 2025-12-09追加: 数字6桁のみ許可
...
-- 🆕 2025-12-09追加: 未使用コードの一意制約（部分インデックス）
...
-- ------------------------------------------------------------
-- sessions（セッション）
-- 🆕 更新（2025-12-09）: device_id追加、stylist_id NULLable（自動識別対応）
-- ------------------------------------------------------------
CREATE TABLE sessions (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  salon_id UUID NOT NULL REFERENCES salons(id) ON DELETE CASCADE,
  device_id UUID REFERENCES devices(id) ON DELETE SET NULL, -- 🆕 セッション開始デバイス
  stylist_id UUID REFERENCES staffs(id) ON DELETE CASCADE,  -- 🆕 NULLable（声紋識別後に設定）
  stylist_match_confidence REAL,                            -- 🆕 スタッフ声紋マッチング信頼度
...
-- ------------------------------------------------------------
-- transcripts（文字起こし）
-- 時間単位はミリ秒で統一（speaker_segmentsと一致）
-- ------------------------------------------------------------
...
-- カラム名は実装と一致: summary, strengths, improvements, metrics
-- ------------------------------------------------------------
...
-- ------------------------------------------------------------
-- success_cases（成功事例）
-- カラム名は実装と一致: approach_text, result, is_active追加
-- ------------------------------------------------------------
...
-- カラム名はtitle/difficultyで統一（実装と一致）
-- ------------------------------------------------------------
...
-- カラム名はmessages/ended_atで統一（実装と一致）
-- ------------------------------------------------------------
...
-- ------------------------------------------------------------
-- operator_admins（運営管理者）
-- 🆕 新規追加（2025-12-09）: Supabase Auth連携
-- staffsと同じパターン: id = auth.users(id)
-- ------------------------------------------------------------
...
-- ------------------------------------------------------------
-- audit_logs（監査ログ）
-- 🆕 更新（2025-12-09）: operator_id追加（運営管理者操作の監査用）
-- ------------------------------------------------------------
CREATE TABLE audit_logs (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  event_type VARCHAR(50) NOT NULL,
  user_id UUID REFERENCES staffs(id),            -- 店舗スタッフの操作
  operator_id UUID REFERENCES operator_admins(id) ON DELETE RESTRICT, -- 🆕 運営管理者の操作
  salon_id UUID REFERENCES salons(id),
  action VARCHAR(100) NOT NULL,                  -- 操作種別（例: salon.suspend, seats.update）
  target_type VARCHAR(50),                       -- 対象種別（salon, operator, system）
  target_id UUID,                                -- 対象ID
  target_name TEXT,                              -- 対象名（検索・表示用）
  resource_type VARCHAR(50),
  resource_id UUID,
  old_value JSONB,
  new_value JSONB,
  details JSONB DEFAULT '{}'::jsonb,             -- 🆕 追加詳細情報
  ip_address INET,
  user_agent TEXT,
  request_id UUID,
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
...
ALTER TABLE operator_admins ENABLE ROW LEVEL SECURITY;  -- 🆕 2025-12-09
ALTER TABLE audit_logs ENABLE ROW LEVEL SECURITY;       -- 🆕 2025-12-09

-- ユーティリティ関数
-- staffs.id = auth.users(id) 構造のため、id = auth.uid() で検索
...
-- 🆕 device_activations ポリシー（2025-12-09追加）
...
-- 🆕 device_seat_history ポリシー（2025-12-09追加）
...
-- ============================================================
-- 🆕 スタッフ声紋識別関数（2025-12-09追加）
-- ============================================================

-- スタッフ声紋マッチング関数
-- セッション中の話者分離後、スタッフを自動識別
...
BEGIN
  ...
  IF NOT FOUND THEN
    RETURN QUERY SELECT false, 0, 'スタッフが見つかりません'::TEXT;
    RETURN;
  END IF;
  ...
  IF current_embedding IS NULL OR NOT is_additional THEN
    ...
    RETURN QUERY SELECT true, 1, '声紋を登録しました'::TEXT;
  ELSE
    -- 追加サンプルで加重平均更新
    -- サンプル数が増えるほど新しいサンプルの重みは小さくなる（最大0.2）
    ...
    RETURN QUERY SELECT true, current_sample_count + 1,
      format('声紋を更新しました（サンプル数: %s）', current_sample_count + 1)::TEXT;
  END IF;
END;
$$;

-- ============================================================
-- 🆕 デバイス管理関数（2025-12-09追加）
-- ============================================================
...
BEGIN
  ...
  -- ユニークなコードが生成されるまでリトライ
  LOOP
    attempt := attempt + 1;
    -- 6桁の数字コードを生成
    new_code := LPAD(FLOOR(RANDOM() * 1000000)::TEXT, 6, '0');

    -- 未使用かつ有効期限内の重複コードがないか確認
    IF NOT EXISTS (
    ...
    IF attempt >= max_attempts THEN
      RAISE EXCEPTION 'アクティベーションコードの生成に失敗しました';
    END IF;
  END LOOP;
...
| staffs | idx_staffs_role | role | B-tree | ロール別検索 |
| staffs | idx_staffs_salon_active | salon_id, is_active | B-tree (部分) | アクティブスタッフ取得 |
| staffs | idx_staffs_voice_embedding | voice_embedding | HNSW | 🆕 スタッフ声紋検索 |
| devices | idx_devices_salon_id | salon_id | B-tree | 🆕 店舗別デバイス取得 |
| devices | idx_devices_status | status | B-tree | 🆕 ステータス別検索 |
| devices | idx_devices_salon_active | salon_id, status | B-tree (部分) | 🆕 アクティブデバイス取得 |
| devices | idx_devices_last_active | last_active_at DESC | B-tree | 🆕 最終アクティブ順取得 |
| devices | devices_identifier_unique | device_identifier | UNIQUE | 🆕 デバイス識別子一意 |
| device_activations | idx_device_activations_code_unique | activation_code | B-tree (部分) | 🆕 未使用コード一意制約 |
| device_activations | idx_device_activations_expires | expires_at | B-tree (部分) | 🆕 有効期限検索 |
| device_seat_history | idx_device_seat_history_device_id | device_id | B-tree | 🆕 デバイス別履歴取得 |
| device_seat_history | idx_device_seat_history_changed_at | changed_at DESC | B-tree | 🆕 変更日時順取得 |
...
| sessions | idx_sessions_device_id | device_id | B-tree | 🆕 デバイス別セッション取得 |
| sessions | idx_sessions_active | device_id, status | B-tree (部分) | アクティブセッション検索 |
| transcripts | idx_transcripts_session_chunk | session_id, chunk_index | B-tree | チャンク順取得 |
| speaker_segments | idx_speaker_segments_session_time | session_id, start_time_ms | B-tree | 時系列取得 |
...
| customers | idx_customers_salon_id | salon_id | B-tree | 店舗別顧客取得 |
| customers | idx_customers_name | name | B-tree (部分) | 名前検索（NULL除外） |
| customers | idx_customers_last_visit | salon_id, last_visit_at | B-tree | 最終訪問順取得 |
| customers | idx_customers_voice_embedding | voice_embedding | HNSW | 声紋類似検索 |
| sessions | idx_sessions_customer_id | customer_id | B-tree | 顧客別セッション取得 |
| operator_admins | idx_operator_admins_email | email | B-tree | 🆕 メール検索 |
| operator_admins | idx_operator_admins_active | is_active | B-tree (部分) | 🆕 アクティブ管理者取得 |
| audit_logs | idx_audit_logs_operator | operator_id, created_at DESC | B-tree | 🆕 運営管理者別監査ログ |
| audit_logs | idx_audit_logs_action | action, created_at DESC | B-tree | 🆕 アクション別監査ログ |
| audit_logs | idx_audit_logs_target | target_type, target_id | B-tree | 🆕 対象別監査ログ |
