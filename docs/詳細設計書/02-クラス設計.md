...
/**
 * スタッフエンティティ
 * 注: staffs.id = auth.users(id) 構造（直接参照）
 */
...
export interface Transcript {
  ...
  readonly startTimeMs: number;  // ミリ秒（DB: start_time_ms INTEGER）
  readonly endTimeMs: number;    // ミリ秒（DB: end_time_ms INTEGER）
...
export interface SuccessCase {
  ...
  readonly approachText: string;       // アプローチ内容（DBに対応）
  readonly successfulTalk: string | null;  // 成功トーク（オプショナル）
  readonly keyTactics: string[] | null;    // 成功要因（オプショナル）
  readonly result: string;             // 結果（DBに対応）
...
export interface CreateSuccessCaseParams {
  ...
  approachText: string;        // 必須
  successfulTalk?: string;     // オプショナル
  keyTactics?: string[];       // オプショナル
  result: string;              // 必須
  soldProduct?: string;
...
export const SuccessCaseDomain = {
  ...
  generateSearchText(successCase: Omit<SuccessCase, 'id' | 'createdAt' | 'updatedAt' | 'embedding'>): string {
    ...
    // アプローチ内容
    parts.push(`アプローチ: ${successCase.approachText}`);
    ...
    // 結果
    parts.push(`結果: ${successCase.result}`);
...
/**
 * スタッフロール
 * ⚠️ 実装に合わせて修正（2025-12-05）: assistant追加
 */
