...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (path.match(/^\/salons\/[^/]+\/devices$/) && req.method === 'POST') {
      ...
      // Generate activation code using the DB function
      const { data: activationData, error: activationError } = await supabaseAdmin
        .rpc('generate_activation_code', {
          device_id_param: device.id,
          created_by_param: session.operator_id,
          validity_hours: 24,
        });

      let activation_code = null;
      let expires_at = null;
      if (!activationError && activationData && activationData.length > 0) {
        activation_code = activationData[0].activation_code;
        expires_at = activationData[0].expires_at;
      }
      ...
      await supabaseAdmin.rpc('record_audit_log', {
        ...
        p_details: { seat_number, activation_code },
        p_ip_address: ip_address,
        p_user_agent: user_agent,
      });

      return respond({ data: { device_id: device.id, activation_code, expires_at, message: 'Device created successfully with activation code.' } }, 201);
    }
